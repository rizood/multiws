...
VALIDITY () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/rizood/permission/main/ipmini | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/rizood/permission/main/ipmini | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
